...
class DatabaseMethods {
  ...
  Stream<QuerySnapshot> searchFoodItems(String query) {
    // Firestore doesn't directly support searches across multiple fields in a single query.
    // This example searches by 'itemName' as before and shows a pattern you might expand.
    // Consider using a search service like Algolia for more complex searches.
    return FirebaseFirestore.instance
        .collection('items')
        .where('itemName', isGreaterThanOrEqualTo: query)
        .where('itemName', isLessThanOrEqualTo: query + '\uf8ff')
        .snapshots();
    // Note: To search other fields, you may need additional queries or a composite index.
  }
}
